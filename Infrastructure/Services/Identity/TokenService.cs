...
public class TokenService : ITokenService
{
    ...
    private async Task<IResult<IDR_002>> GenerateTokensAndUpdateUser(AppUser user, string ipAddress)
    {
        ...
        var expiryTime = DateTime.UtcNow.AddDays(_jwtSettings.TokenExpirationInDays);
        user.SetRefreshToken(refreshToken, expiryTime);
        await _userManager.UpdateAsync(user);
        var response = new IDR_002() { Token = token, RefreshToken = user.RefreshToken, RefreshTokenExpiryTime = (DateTime)user.RefreshTokenExpiryTime };
